...
| 107 | ⚠️ **SIGTI necesita credencial de sistema en `ARGOS_API`.** El padrón completo pasó a exigir **token de sistema** —y con razón: 193 personas con número de identidad y correo no las debe abrir el token de cualquier empleado—. SIGTI hoy reenvía el token de la persona, así que la sincronización del espejo devuelve `403` | Sin esto el espejo deja de poblarse. El secreto se declara en la configuración de `ARGOS_API`, no en `SICOVBD`, que no se escribe | **PO** — decidir el secreto; el resto es código |
