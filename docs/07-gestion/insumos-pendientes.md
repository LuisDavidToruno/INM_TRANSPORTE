...
| 107 | ✅ **CERRADO el 2026-08-31.** El secreto de sistema se generó al azar y quedó en la configuración local de los dos lados — en `ARGOS_API` como consumidor `sigti`, en SIGTI como `Argos:Secreto`—. **Ninguno de los dos archivos se versiona**, y el secreto no se imprimió en ninguna parte. SIGTI dejó de reenviar el token de la persona para bajar el padrón: ese volumen de datos de terceros no lo abre la cuenta de quien sólo quería pedir un vehículo | Verificado en vivo: `POST /organigrama/sincronizar` → **193 personas, 193 con puesto, 10 puestos distintos**, y las 14 asignaciones propias de SIGTI sobrevivieron a la sincronización. ⚠️ **Rotarlo es una línea en dos archivos**, y hay que hacerlo el día que se despliegue fuera de desarrollo | Cerrado |
| 108 | ⚠️ **`HB23-01` — la fila 23 de la matriz de permisos se contradice con su propia nota al pie.** «Mantener expediente y vencimientos del vehículo» le da a `ACT-11` Encargado de Mantenimiento una **`E³`**, y la nota 3 dice *«informa disponibilidad y estado técnico; no decide la asignación»* — que describe consulta, no ejecución, y parece redactada para la fila 4 | El alta de vehículo tomó **lo restrictivo**: `ACT-11` no está en la lista que ejecuta. Si la `E` es la correcta, hoy le falta un permiso que la matriz le da. Su estado técnico ya lo mueve por `/flota/{id}/estado`, así que no queda bloqueado | **PO** — decidir si la `E³` de la fila 23 es ejecución o consulta |
...
| 102 | **Qué valor lleva `aviso.canal`** — `SoloBandeja`, `CorreoInstitucional` o `MensajeDeTexto` | **El mecanismo ya está**: es parámetro con vigencia y doble control, y elegirlo es cargar una clave, no tocar código. Sin ella el aviso resulta `SinCanalConfigurado` y se declara — no se supone uno «razonable», porque eso produce un sistema que cree haber avisado y una persona que nunca recibió nada. **`SoloBandeja` es respuesta legítima**: en delegaciones sin señal el correo y el SMS no llegan (`P-5`). Los otros dos, además de la decisión, exigen construir el canal y contratar servidor o pasarela | §5.3.B.3, `RN-60`, `RN-63` |
