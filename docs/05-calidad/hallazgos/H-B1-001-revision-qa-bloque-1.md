...
| `HB1-20` | **Cerrado el 2026-08-27.** La leyenda `Cfg.` del [`README` de reglas](../../01-negocio/reglas/README.md) define ahora los **tres** valores: `No`, `Sí` —hay parámetro configurable pero el bloqueo no se apaga; 61 filas— y `Sí*` —el bloqueo mismo se puede apagar—. `RN-12` pasó de `Sí*` a `Sí`, que es lo que su ficha dice: *«No el bloqueo. Sí el catálogo `tipo_ausencia`»*. Hoy **[`RN-16`](../../01-negocio/reglas/RN-16-seguro-y-revision-mecanica.md) es la única `Sí*`** |
| `HB1-24` | **Cerrado el 2026-08-27.** [`RN-04`](../../01-negocio/reglas/RN-04-anulacion-como-asiento-reverso.md) decía que el borrador descartado se registra *«sin conservar el contenido»*; la máquina de estados, **autoridad**, dice que pasa a `ANULADA` con motivo y que *«tampoco es un borrado físico»*. Manda la máquina: `ANULADA` tiene `INV-40` a `INV-43` —motivo tipificado y autor— que un expediente vacío no podría satisfacer, y borrarlo contradice el principio **P-3**. Se retiró el `[C]` por ser la pregunta equivocada y se abrió la correcta: **el plazo de depuración del dato personal** de un borrador de `M-17`. Corregido también el añadido del hallazgo — [`PR-01`](../../01-negocio/procesos/PR-01-movilizacion-institucional.md) §10 ahora distingue si hubo consumo: sin consumo `T-15`, con consumo `T-16`, porque `EF-06` prohíbe anular una misión que gastó |
| `HB1-25` | **Cerrado el 2026-08-27.** El objetivo 6 de [`vision-de-producto.md`](../../00-vision/vision-de-producto.md) tiene unidad, umbral y línea base: el paquete de [`RN-98`](../../01-negocio/reglas/RN-98-paquete-de-evidencia-por-vehiculo-o-periodo.md) en **≤ 15 minutos**. El objetivo 10 dejó de medir *«ubicación actualizada ≥ 90 %»* —que **castigaba el caso que el producto dice atender**, porque una misión de cuatro días sin señal no puede tenerla por diseño— y pasa a **antigüedad mediana del último dato conocido**, con las zonas sin cobertura excluidas explícitamente |
