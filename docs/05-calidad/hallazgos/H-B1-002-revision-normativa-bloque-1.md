...
| **Estado** | ****Los 20 corregidos y verificados.** Ninguno abierto** — desglose en la sección siguiente |
...
| `HN1-09` | **Cerrado el 2026-08-26** con [`RN-98`](../../01-negocio/reglas/RN-98-paquete-de-evidencia-por-vehiculo-o-periodo.md) — la evidencia se entrega también por **vehículo** y por **período**, ensamblando expedientes, conciliaciones, hallazgos, constataciones, cambios de parámetro e intentos bloqueados del alcance. Reproducible: se reconstruye con el paquete normativo congelado de cada misión, no con las tablas vigentes al exportar. La verificación quedó `[I]` —el formato y los dos ejes son implicación de requerimiento de [`NRM-01`](../../01-negocio/normativa/NRM-01-control-interno-tsc.md), no articulado— y `[C]` el plazo de conservación |
| `HN1-18` | **Cerrado el 2026-08-26.** Eran **ocho** materias, no una. Cuatro pasaron a regla: [`RN-99`](../../01-negocio/reglas/RN-99-constatacion-fisica-de-la-flota.md) constatación física de la flota, [`RN-100`](../../01-negocio/reglas/RN-100-permisos-por-puesto-no-por-persona.md) permisos por puesto, [`RN-101`](../../01-negocio/reglas/RN-101-cierre-de-asignacion-de-puesto.md) cierre de asignación con custodias, [`RN-102`](../../01-negocio/reglas/RN-102-reporte-publico-de-flota.md) reporte público de flota. La quinta —guía de actuación en accidente sin conexión— entró en [`RN-43`](../../01-negocio/reglas/RN-43-captura-de-campo-sin-conectividad.md), que es donde se declara qué funciona sin red. Las tres restantes **no eran huecos**: multas y siniestros son M-12, mantenimiento es M-11, y el TAG está bloqueado por el insumo **#24**. El [`README` de reglas](../../01-negocio/reglas/README.md) las declara ahora en una sección propia, que es lo que el hallazgo pedía de raíz: *«no hay forma de distinguir un hueco de una postergación»*. **Corregida además la escalada del propio hallazgo**: afirmaba `[V]` por *«NOGECI V-15 y Circular CGR-010-2026»*; solo la Circular está `[V]` — la tabla NOGECI de `NRM-01` está `[P]` |
...
| `HN1-13` | **Cerrado el 2026-08-26.** La condición 3 de `BD-02` salió a un bloqueo propio, **[`BD-12`](../../03-arquitectura/estados/orden-de-mision.md)**, gobernado por [`RN-11`](../../01-negocio/reglas/RN-11-restricciones-medicas-del-motorista.md): bloqueo para las restricciones que el catálogo tipifica como incompatibilizantes, advertencia con acuse para el resto, y advertencia genérica para las que llegan sin clasificar. **No fue la regla ganándole a la autoridad**: fui a [`DP-001` D-12](../../07-gestion/decisiones-de-producto/DP-001-fronteras-con-sistemas-existentes.md), de donde `BD-02` toma su «sin excepción configurable», y decide exactamente dos cosas —categoría insuficiente y licencia vencida en el rango—. Las restricciones médicas nunca estuvieron ahí; heredaron una etiqueta que nadie les dio. Separarlas en `BD-nn` propio en vez de matizar dentro de `BD-02` **conserva íntegro** ese «sin excepción» donde el PO sí lo decidió. Se corrigió además el insumo citado: decía **#23**, que está cerrado y era la descarga de los PDF; el correcto es el **#42**, el catálogo de la DNVT que no tiene fuente pública. `BD-12` quedó listado en las precondiciones de `T-08` y en las revalidaciones de `T-12` y `T-17` |
| `HN1-11` | **Cerrado el 2026-08-27** con [`RN-103`](../../01-negocio/reglas/RN-103-matricula-vigente-para-despachar.md). El bloqueo por matrícula vencida tiene regla, y con su **nivel real**: `[V]` que el Instituto de la Propiedad lleva el Registro Vehicular y que la ficha exige registrar y alertar; **`[I]` que el vencimiento deba bloquear**, porque [`NRM-06`](../../01-negocio/normativa/NRM-06-transito-y-licencias.md) no lo dice y el articulado no se extrajo. Se resolvió el contraste que el hallazgo señalaba: el seguro no bloquea porque **la institución no siempre puede resolverlo**; la matrícula sí bloquea porque **es un trámite propio sobre un bien propio**. Y como el nivel es `[I]`, el bloqueo quedó **configurable con el interruptor encendido por defecto** —el opuesto de [`RN-16`](../../01-negocio/reglas/RN-16-seguro-y-revision-mecanica.md)—, en vez de cablear una obligación no verificada |
| `HN1-16` | **Cerrado el 2026-08-27, y no por elección entre las dos posturas: no chocaban.** El revisor las dio por irreconciliables. Al leerlas juntas, mezclaban dos cosas: **delegar** da la facultad a quien no la tendría y exige un acto ([`RN-07`](../../01-negocio/reglas/RN-07-delegacion-de-autorizacion.md)); **escalar** enruta la decisión a quien **ya es competente por jerarquía** y no crea nada. [`RN-02`](../../01-negocio/reglas/RN-02-escalamiento-de-autorizacion.md) ya escalaba al inmediato superior sin delegación cuando el solicitante era el autorizador — **si eso vale ahí, vale ante una ausencia registrada**. Lo que la regla protegía sigue protegido, porque el acto es el **nombramiento en el puesto**; lo prohibido pasa a ser lo que la frase decía mal: **saltarse niveles** |
| `HN1-17` | **Cerrado el 2026-08-27.** [`RN-25`](../../01-negocio/reglas/RN-25-salvoconducto-con-folio-y-qr.md) separó lo decidido de lo que no lo está. **Siguen siendo bloqueo duro** el documento con sus seis elementos y la existencia de un punto de verificación; pasa a **configuración** si ese punto es **público en internet o interno de la institución**, que es exactamente el pendiente **G**. No se retiró el control: se retiró la **suposición** de que solo puede ejercerse por una vía — y de paso el supuesto de que hay un servidor publicable desde un despliegue on-premise. Corregido también el nivel: era `[V]` a secas sobre un enunciado que cita la premisa rectora 4 de `CLAUDE.md`, que es premisa de proyecto; **el formato concreto es `[I]`** |

### Ninguno abierto

**Los veinte están corregidos y verificados contra los artefactos vivos.**
