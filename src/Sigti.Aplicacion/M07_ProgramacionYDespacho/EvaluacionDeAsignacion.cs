    ...
    IParametrosDeLaInstitucion parametros)
{
        ...
        CancellationToken cancelacion = default)
    {
        ...
        var contradichas = hayConduccionNocturna
            ? conductor.Licencia.Restricciones.Where(r => r.Contains("nocturn", StringComparison.OrdinalIgnoreCase)).ToList()
            : [];

        var habilitacion = ReglasDeHabilitacion.Evaluar(
            conductor.Licencia, vehiculo.Ficha, ventana, matriz, conocidoAl, contradichas);
        ...
        return new ResultadoDeAsignacion(
            Habilita: habilitacion.Habilita && documentacion.Habilita,
            ...
            RestriccionEnConflicto: contradichas.FirstOrDefault(),
            ...
            ConductoresQueHabilitan: flota.Conductores
                ...
                .Where(c => ReglasDeHabilitacion.Evaluar(
                    c.Licencia, vehiculo.Ficha, ventana, matriz, conocidoAl,
                    hayConduccionNocturna
                        ? c.Licencia.Restricciones.Where(r => r.Contains("nocturn", StringComparison.OrdinalIgnoreCase)).ToList()
                        : []).Habilita)
                .Select(c => c.Id)
                .ToList(),
            ...
            VehiculosQueHabilita: flota.Vehiculos
                ...
                .Where(v => ReglasDeHabilitacion.Evaluar(
                    conductor.Licencia, v.Ficha, ventana, matriz, conocidoAl, contradichas).Habilita)
                .Select(v => v.Id)
                .ToList());
    }
